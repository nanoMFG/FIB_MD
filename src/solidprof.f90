program solidprof
  ...
  real :: s_ro, s_dr, s_dz, s_dt, pi, halfuc, s_rxi, s_txi, diffr
  character(30) :: fn
  ...
  do fi=ti,tf,df
      ...
      do i=1,Natm
          ...
          if (tst(i).eq. 0) then
              diffx = Xf(i,:)-X(i,:)
              diffr = sqrt(sum(diffx(:)**2))
              
              if(diffr .lt. halfuc) then
                  tst(i) = 1
                  dt = (fi-ti+df)*0.2E-15
                  ...
                  if(s_ri .le. s_nr .and. s_ti .le. s_nt .and. s_zi .le. s_nz) then
                      ...
                      s_rr(s_dum,4:6) = s_rr(s_dum,4:6) + diffx/dt
  ...
  do i=1,s_nz
      do j=1,s_nt
          do k=1,s_nr
              ...
              if(s_rr(s_cnt,12) .ne. 0) then
                  s_rr(s_cnt,4:6)=s_rr(s_cnt,4:6)/s_rr(s_cnt,12)
                  s_rr(s_cnt,7)=s_rr(s_cnt,7)/s_rr(s_cnt,12)
              end if

              s_symm(sym_cnt,4:7) = s_symm(sym_cnt,4:7) + s_rr(s_cnt,4:7)
  ...
  print *, 'writing...'
  open(52, file='solidprof.dat')
  open(53, file='symmsolid.dat')
  ...
  do i=1,sym_cnt
      write(53,"(7E20.10)")s_symm(i,1:3), s_symm(i,4:7)/(360.0/s_dt)
  end do
